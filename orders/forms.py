...
class UploadOrderForm(forms.Form):
    ...
    수주일자 = forms.DateField(required=True, help_text="*형식 : (2020-**-**) ",)
    거래처코드 = forms.CharField(
        max_length=20, required=True, help_text="*거래처 코드로 입력해주시길 바랍니다."
    )
    현장명 = forms.CharField()
    납품요청일 = forms.DateField(required=False, help_text="*형식 : (2020-**-**) ")
    특이사항 = forms.CharField(required=False, widget=forms.TextInput(attrs={"size": "38"}))
